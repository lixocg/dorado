...
public class LitigationTransitRest {
	...
	@Autowired
	private LitigationTransmissionLogService transmissionLogService;
	...
	public R<Void> executeDb2Root(@PathVariable("count") int count) {
		// 初始化log表
		LitigationTransmissionLog transmissionLog = transmissionLogService.initialize();
		Date updateDate = transmissionLog.getUpdatedate();
		ExecutorService executorService = Executors.newFixedThreadPool(count);
		// 查询待处理的诉讼主表数据条数
		Integer countNum = litigationTransitService.getCorpLitigationCount(updateDate);
		...
		if (countNum > 0) {
			...
			while (curPage * pageSize < countNum) {
				curPage++;
				executorService.execute(new LitigationTransitJob(litigationTransitService, curPage, pageSize,updateDate));
			}
		}
	...
	public R<Void> executeDb2HistoryRoot(@PathVariable("count") int count) {
		LitigationTransmissionLog lastestLog = transmissionLogService.getLastestLog();
		Date updateDate = lastestLog.getUpdatedate();
		
		ExecutorService executorService = Executors.newFixedThreadPool(count);
		// 查询待处理的诉讼进程表数据条数
		Integer countNum = litigationTransitService.getLitigationHistroyCount(updateDate);
		...
		if (countNum > 0) {
			...
			while (curPage * pageSize < countNum) {
				curPage++;
				executorService.execute(new LitigationHistoryTransitJob(litigationTransitService, curPage, pageSize,updateDate));
			}
		}
	...
	public R<Void> executeSqlServer(@PathVariable("count") int count) {
		...
		ExecutorService executorService = Executors.newFixedThreadPool(count);
		// 查询待处理的诉讼平台数据条数
		Integer countNum = litigationTransitService.getSqlserverCorpLitigationCount(updateDate);
		...
		if (countNum > 0) {
			...
			while (curPage * pageSize < countNum) {
				curPage++;
				executorService.execute(new LitigationSqlServerTransitJob(litigationTransitService, curPage, pageSize,updateDate));
			}
		}
